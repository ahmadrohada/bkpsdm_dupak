...
if( (isset($_POST['cari'])) | (isset($_GET['cari']))){
		if($_POST['cari']!=null){
			...
			$txtcari=$_GET['cari'];
		//pencarian data
		$page['jika'] = "dt_pegawai.nip_baru = dt_pak.nip_baru and (dt_pegawai.nip_baru = '$txtcari' or nama LIKE '%$txtcari%' or dt_pak.no_pak = '$txtcari') and SUBSTRING_INDEX(SUBSTRING_INDEX(no_pak,'/',3),'/',-1) = 'TP.GURU'";
		$all['jika'] = "dt_pegawai.nip_baru = dt_pak.nip_baru and (dt_pegawai.nip_baru = '$txtcari' or nama LIKE '%$txtcari%' or dt_pak.no_pak = '$txtcari') and SUBSTRING_INDEX(SUBSTRING_INDEX(no_pak,'/',3),'/',-1) = 'TP.GURU' ";
		$_SESSION['cari']="&cari=".$txtcari; //menyiapkan GET cari untuk pagination
		$_SESSION['nama_file']= $nama_file;
} else {
		//defaul data	
		$page['jika'] = "dt_pegawai.nip_baru = dt_pak.nip_baru and SUBSTRING_INDEX(SUBSTRING_INDEX(no_pak,'/',3),'/',-1) = 'TP.GURU'";
		$all['jika'] = "dt_pegawai.nip_baru = dt_pak.nip_baru and SUBSTRING_INDEX(SUBSTRING_INDEX(no_pak,'/',3),'/',-1) = 'TP.GURU'";
}
